...
public class PruebaService {
    ...
    public int crearCodigo(MultipartFile archivo, String respuesta) {
        try {
            // Crear la solicitud HTTP con el archivo y la respuesta
            ...
            // Realizar la petición POST a la URL del servicio de código
            String url = "http://localhost:8002/codigo/crear"; // Cambia la URL según corresponda
            ResponseEntity<Integer> response = restTemplate.exchange(url, HttpMethod.POST, requestEntity, Integer.class);

            if (response.getStatusCode() == HttpStatus.OK) {
                return response.getBody(); // Devuelve el ID del código creado
            } else {
                return -1; // O devuelve un valor negativo para indicar un error
            }
        } catch (Exception error) {
            return -1; // O devuelve un valor negativo para indicar un error
        }
